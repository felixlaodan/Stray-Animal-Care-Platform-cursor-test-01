...
const load = () => {
  ...
  getUploadRecords(params).then(res => {
    ...
    if (responseData.code === '200' && responseData.data) {
      ...
      data.records = responseData.data.records;
...
const add = () => {
  ...
  createUploadRecord(formData).then(res => {
    const responseData = res.data;
...
const update = () => {
  ...
  updateUploadRecord(formData).then(res => {
    const responseData = res.data;
    if (responseData.code === '200') {
      data.dialogFormVisible = false;
      ElMessage.success("操作成功");
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm('删除数据后无法恢复，确认删除吗？', '删除确认', { type: 'warning' }).then(() => {
    deleteUploadRecord(row.uploadRecordId).then(res => {
      const responseData = res.data;
      if (responseData.code === '200') {
        ElMessage.success("操作成功");
        load();
...
const handleDeleteBatch = () => {
  ...
  ElMessageBox.confirm('删除数据后无法恢复，确认删除吗？', '删除确认', { type: 'warning' }).then(() => {
    deleteUploadRecordBatch(data.ids).then(res => {
      const responseData = res.data;
      if (responseData.code === '200') {
        ElMessage.success("操作成功");
        load();
